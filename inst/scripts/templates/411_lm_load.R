#' ---
#' title: "Greenhouse Gas Cumulative Loads Linear Modeling"
#' author: "Ezra Moses"
#' date: "`r Sys.time()`"
...
if (!dir.exists('data/04_analyzed/load')) {
  dir.create('data/04_analyzed/load', recursive = TRUE)
}
...
mod0_dat <- dat %>%
  ...
  rename(co2 = co2_kg_ha_load, n2o = n2o_kg_ha_load,
         ch4 = ch4_kg_ha_load, nh3 = nh3_kg_ha_load) %>%
...
# Export to CSV
write.csv(mod0_dat, 'data/04_analyzed/load/mod0_levene_shapiro.csv',
          row.names = FALSE)
...
pdf('data/04_analyzed/load/mod0_residual_plots.pdf',
    width = 12, height = 20)
plot(mod0_resplots)
dev.off()
...
#' Export qq plots
pdf('data/04_analyzed/load/mod0_qq_plots.pdf',
    width = 12, height = 20)
plot(mod0_qqplots)
dev.off()
...
write.csv(mod0_anovas, 'data/04_analyzed/load/mod0_anova.csv', row.names = F)
...
saveRDS(mod0_dat_long, 'data/04_analyzed/load/mod0_Rdata.RDS')
...
mod1_dat %>% bind_rows(.id = 'site') %>%
  write.csv('data/04_analyzed/load/mod1_levene_shapiro.csv', row.names = FALSE)
...
#' Export pdf of residual plots
mod1_resplot_dir <- 'data/04_analyzed/load/mod1_residual_plots'
...
#' Export qq plots
mod1_qqplot_dir <- 'data/04_analyzed/load/mod1_qqplots'
...
#' Export ANOVA data to CSV
write.csv(mod1_anovas, 'data/04_analyzed/load/mod1_anova.csv', row.names = F)
...
datatable(head(mod1_dat_long, 100) %>% mutate(across(where(is.numeric), ~round(.x, 3))),
          rownames = F, options = list(scrollX = T, scrollY = '200px'))
saveRDS(mod1_dat_long, 'data/04_analyzed/load/mod1_Rdata.RDS')
...
file.edit('code/412_lm_co2_equiv.R')
